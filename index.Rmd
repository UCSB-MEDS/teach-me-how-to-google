...
penguins2 <- penguins %>% 
  ...
  mutate(spp2 = str_to_lower(species))
...
<span style = 'font-size: 100%;'>`r fontawesome::fa("plus", fill = "#05859B", a11y = "sem")`  Read multiple search results</span>
  ...
  - <span style = 'font-size: 75%;'>sometime one explanation will make sense in a way that another explanation will not</span>
...
`r fontawesome::fa("angle-right", fill = "#05859B", a11y = "sem")` I would love to expand on this, with a focus on data wrangling and visualization. Ping me with ideas!
  
`r fontawesome::fa("angle-right", fill = "#05859B", a11y = "sem")` In the meantime, keeping a running vocab list for yourself might be massively helpful
...
<span style = 'font-size: 75%;'>[Pseudocode](https://en.wikipedia.org/wiki/Pseudocode) is a step-by-step written "outline" of your code that you can then transcribe into a programming language (in our case, likely R). It is written in plain language (e.g. English), but uses the structural conventions of a programming language.</span>
...
for(each species of penguin in penguins){
  get species name
  filter out NAs
  calculate the mean bill length for that species
  print species name and mean bill length
}
...
# unique_spp_names <- unique(penguins$species)
# 
# for(i in 1:length(unique_spp_names)){
#   
#   spp <- as.character(unique_spp_names[[1]])
#   
#   mean_bill_length <- penguins %>% 
#     filter(species == spp, na.rm = TRUE) %>% 
#     mean(bill_length_mm)
#   
#   print(mean_bill_length)
#   
#   #print(spp, "penguins have a mean bill length of:", mean_bill_length, "mm")
#   
# }
```
...
```{r echo = FALSE, eval = FALSE}
##~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
##                        Without Explaining Variables                      ----
##~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

determineShippingSpeed <- function(shippingRoute, shippingMethod){
  
  # if the shipping route is very short, or if it's by air
  if(shippingRoute < 20 || shippingMethod == "Air")
    print("Same Day Shipping") else{
      print("Other")
    }
}
...
determineShippingSpeed <- function(shippingRoute, shippingMethod){
  ...
  routeIsVeryShort <- shippingRoute < 20
  isByAir <- "Air"
  shippingMethod <- isByAir
  
  # if the shipping route is very short, or if it's by air
  if(shippingRoute == routeIsVeryShort || shippingMethod == isByAir)
    print("Same Day Shipping") else{
      print("Other")
    }
}
...
Bummer, our code broke. But informative error message to the rescue! Seems like we may have a spelling error for the input values provided to either the `from` and/or `to` arguments. Let's fix and try again:
...
Copy and paste the following coding challenges into an R script or Rmd file. Problems may have more than one error or missing code (be mindful, you might not even get an error message) to identify and fix. Remember: running code line-by-line is one of the best ways to isolate errors!
